...
def get_character_data(character_name):
    """Получение данных персонажа с обратной совместимостью"""
    if character_name in CHARACTERS_DB:
        data = CHARACTERS_DB[character_name].copy()
        # Убедимся, что hitbox_size есть
        if "hitbox_size" not in data:
            data["hitbox_size"] = (60, 120)

        # Создаем frame_ranges и animation_speeds из animations для обратной совместимости
        if "animations" in data and ("frame_ranges" not in data or "animation_speeds" not in data):
            data["frame_ranges"] = {}
            data["animation_speeds"] = {}
            for anim_name, (frames, speed) in data["animations"].items():
                data["frame_ranges"][anim_name] = frames
                data["animation_speeds"][anim_name] = speed
...
def get_stand_data(character_name):
    """Получение данных стенда с обратной совместимостью"""
    data = STANDS_DB.get(character_name, None)
    if data and "animations" in data and ("frame_ranges" not in data or "animation_speeds" not in data):
        data["frame_ranges"] = {}
        data["animation_speeds"] = {}
        for anim_name, (frames, speed) in data["animations"].items():
            data["frame_ranges"][anim_name] = frames
...
def get_action_animation_speed(character_name, action):
    if character_name in CHARACTERS_DB:
        ...
        if "animations" in character_data and action in character_data["animations"]:
            return character_data["animations"][action][1]
